...
{
    /// <summary>
    /// 验证是不是long类型数字 (int/short/long只要满足一个就行, 大包含小原则)
    /// </summary>
    ...
    public sealed class LongAttribute : ValidationAttribute
    {
        ...
        /// <param name="u">是否只验证正数, 负数和一律验证不通过, 默认不是</param>
        /// <param name="zero">是否排除 0 , 当u设置为true的时候, 该参数才有效, 默认不排除</param>
        public LongAttribute(string errorMsg, bool u = false, bool zero = false)
        {
